...
@section('title', 'Complete Assignments')
...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div class="mb-4 md:mb-0">
                <div class="flex items-center mb-2">
                    <a href="{{ route('cloth-assignments.index') }}" class="mr-4 text-green-600 hover:text-green-800">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                        </svg>
                    </a>
                    <h1 class="text-3xl font-bold text-gray-900">{{ __('Complete Assignments') }}</h1>
                </div>
                <p class="text-gray-600">{{ __('View all completed work assignments') }}</p>
            </div>
    ...
    <div class="bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl shadow-lg p-6 mb-8 text-white">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-green-100 text-sm font-medium">{{ __('Complete Assignments') }}</p>
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <h2 class="text-xl font-semibold text-gray-900">{{ __('Complete Assignments') }}</h2>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                ...
                                <a href="{{ route('cloth-assignments.edit-customer', $cust->cus_id) }}" class="text-indigo-600 hover:text-indigo-900 p-2 rounded-lg hover:bg-indigo-50 transition-colors duration-200" title="Update Assignments">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('No complete assignments') }}</h3>
                                <p class="text-gray-500 mb-4">{{ __('No assignments have been completed yet.') }}</p>
                                <a href="{{ route('cloth-assignments.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition-colors duration-200">
                                    {{ __('Create New Assignment') }}
                                </a>
                            </div>
                        </td>
...
<script>
    ...
    function viewCustomerAssignments(customerId) {
        ...
        alert('Viewing assignments for customer ID: ' + customerId);
