...
@section('title', 'Edit Customer Assignments')
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ __('Edit Assignments for') }} {{ $customer->cus_name }}</h1>
                ...
                <div class="flex space-x-3">
                    <a href="{{ route('cloth-assignments.complete-view') }}"
                       class="inline-flex items-center px-4 py-2 bg-gray-600 text-white font-semibold rounded-lg hover:bg-gray-700 transition-colors duration-200">
                        ...
                        {{ __('Back to Complete Assignments') }}
                    </a>
                </div>
            </div>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('cloth-assignments.update-customer', $customer) }}" method="POST" class="space-y-6">
            @csrf
            @method('PATCH')

            @foreach($clothMeasurements as $clothMeasurement)
                <div class="bg-white rounded-2xl shadow-xl p-6">
                    <h3 class="text-xl font-semibold text-gray-900 mb-6">
                        {{ __('Cloth Measurement') }} #{{ $clothMeasurement->cm_id }}
                        <span class="text-sm font-normal text-gray-500">({{ $clothMeasurement->size }})</span>
                    </h3>

                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        @foreach($clothMeasurement->clothAssignments as $assignment)
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select name="assignments[{{ $assignment->ca_id }}][F_emp_id]"
                                            ...
                                            @foreach($employees as $employee)
                                                @if($assignment->work_type === 'cutting' && $employee->role === 'cutter' && $employee->type === 'cloth')
                                                    <option value="{{ $employee->emp_id }}"
                                                            {{ $assignment->F_emp_id == $employee->emp_id ? 'selected' : '' }}>
                                                        {{ $employee->emp_name }} ({{ ucfirst($employee->role) }})
                                                    </option>
                                                @elseif($assignment->work_type === 'salaye' && $employee->role === 'salaye' && $employee->type === 'cloth')
                                                    <option value="{{ $employee->emp_id }}"
                                                            {{ $assignment->F_emp_id == $employee->emp_id ? 'selected' : '' }}>
                                                        {{ $employee->emp_name }} ({{ ucfirst($employee->role) }})
                                                    </option>
                                    ...
                                    <div>
                                        ...
                                        <select name="assignments[{{ $assignment->ca_id }}][status]"
                                            ...
                                            <option value="complete" {{ $assignment->status === 'complete' ? 'selected' : '' }}>
                                                {{ __('Complete') }}
                                            </option>
                                        </select>
                                    </div>
...
<script>
    ...
    function confirmDeleteAssignment(assignmentId, workType) {
        if (confirm(`Are you sure you want to delete this ${workType} assignment? This action cannot be undone.`)) {
            ...
            form.method = 'POST';
            form.action = `/cloth-assignments/delete-assignment/${assignmentId}`;
