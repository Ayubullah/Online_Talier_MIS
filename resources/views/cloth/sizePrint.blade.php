...
<head>
    ...
    <title>Cloth Size Print - {{ $cloth->cm_id }}</title>
        ...
        <div class="col-12">
            ...
            <table class="table-bordered border-black w-100 text-black" style="font-size: 20px;">
                ...
                <tr class="text-center text-black">
                    <td style="border-right:none;">{{ $cloth->customer->cus_name ?? 'N/A' }}</td>
                    <td class="border-white p-2">مشتری</td>
                    <td class="border-white">{{ $cloth->customer->phone->pho_no ?? 'N/A' }}</td>
                    <td>شماره</td>
                </tr>
                <tr class="text-center">
                    <td class="Inc_table1">N/A</td>
                    <td>نوع</td>
                    <td class="Inc_table1">{{ $cloth->cm_id }}</td>
                    <td>ID</td>
                </tr>
                <tr class="text-center">
                    <td class="Inc_table1">{{ $cloth->size ?? 'N/A' }}</td>
                    <td>سایز</td>
                    <td class="Inc_table1">{{ $cloth->cloth_rate ?? '0' }}</td>
                    <td>نرخ</td>
                </tr>
                <tr class="text-center">
                    <td class="Inc_table" colspan="2">فرمایش لباس</td>
                    <td class="Inc_table1">{{ $cloth->Height ?? '0' }}</td>
                    <td>قد</td>
                </tr>
                <tr class="text-center">
                    <td class="Inc_table">{{ $cloth->Shoulder ?? '0' }}</td>
                    <td class="Inc_table">شانه</td>
                    <td class="Inc_table1">{{ $cloth->Armpit ?? '0' }}</td>
                    <td>بغل</td>
                </tr>
                <tr class="text-center">
                    <td class="Inc_table">{{ $cloth->Sleeve ?? '0' }}</td>
                    <td class="Inc_table">آستین</td>
                    <td class="Inc_table1">{{ $cloth->Collar ?? 'N/A' }}</td>
                    <td>یخن</td>
                </tr>
                <tr class="text-center">
                    <td class="Inc_table">{{ $cloth->Kalar ?? 'N/A' }}</td>
                    <td class="Inc_table">انواع کالر</td>
                    <td class="Inc_table1">{{ $cloth->Daman ?? 'N/A' }}</td>
                    <td>انواع دامن</td>
                </tr>
                <tr class="text-center">
                    <td class="Inc_table">{{ $cloth->Shalwar ?? 'N/A' }}</td>
                    <td class="Inc_table">انواع تنبان</td>
                    <td class="Inc_table1">{{ $cloth->order_status ?? 'pending' }}</td>
                    <td>حالت</td>
                </tr>
                <tr>
                            ...
                            @if($cloth->customer && $cloth->customer->phone && $cloth->customer->phone->pho_no)
                                @php
                                    try {
                                        ...
                                        echo $barcode->getBarcodeHTML((string)$cloth->customer->phone->pho_no, 'C128', 2.5, 35, 'black', true);
                                    } catch (\Exception $e) {
                                        ...
                                        echo '<p style="margin-top: 5px; font-size: 12px; text-align: center;">' . htmlspecialchars($cloth->customer->phone->pho_no) . '</p>';
                                    }
                                @endphp
                            @else
                            ...
                            @if($cloth->cm_id)
                                @php
                                    try {
                                        $barcode = new \Milon\Barcode\DNS1D();
                                        // Display barcode with integrated text, full width
                                        echo $barcode->getBarcodeHTML((string)$cloth->cm_id, 'C128', 2.5, 35, 'black', true);
                                    } catch (\Exception $e) {
                                        echo '<div style="height: 30px; width: 100%; background: repeating-linear-gradient(90deg, #000 0px, #000 2px, #fff 2px, #fff 4px); -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; margin: 0 auto;"></div>';
                                        echo '<p style="margin-top: 5px; font-size: 12px; text-align: center;">' . htmlspecialchars($cloth->cm_id) . '</p>';
                                    }
                                @endphp
                            @else
                                <div style="height: 30px; width: 100%; background: repeating-linear-gradient(90deg, #000 0px, #000 2px, #fff 2px, #fff 4px); -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; margin: 0 auto;"></div>
                ...
                <tr>
                    <td colspan="4" class="text-end font-monospace">
                        <span>
                            @if($cloth->O_date && $cloth->R_date)
                                {{ $cloth->O_date }}--
                                {{ $cloth->R_date }}
                                @php
                                    try {
                                        $date1 = new DateTime($cloth->O_date);
                                        $date2 = new DateTime($cloth->R_date);
                                ...
                                @endphp
                            @else
                                {{ $cloth->O_date ?? 'N/A' }}--{{ $cloth->R_date ?? 'N/A' }}
                            @endif
                        </span>
