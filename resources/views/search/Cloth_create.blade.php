...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8 px-4">
    ...
    @if(isset($selectedData) && $selectedData->count() > 0)
        <div class="bg-white rounded-3xl shadow-2xl p-6 mb-8 border border-white/20 backdrop-blur-sm">
            <div class="bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 px-6 py-4 mb-6 rounded-2xl">
                ...
                <p class="text-purple-100 mt-1">{{ __('Review and update the selected cloth orders below') }}</p>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white rounded-lg overflow-hidden shadow-lg">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($selectedData as $index => $cloth)
                            <tr class="hover:bg-gray-50 {{ $index === 0 ? 'bg-blue-50' : '' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                                    ${{ number_format($cloth->cloth_rate, 2) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                        @if($cloth->order_status === 'complete') bg-green-100 text-green-800
                                        @elseif($cloth->order_status === 'pending') bg-yellow-100 text-yellow-800
                                        @else bg-gray-100 text-gray-800 @endif">
                                        {{ $cloth->order_status_display }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $cloth->formatted_order_date ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="{{ $cloth->is_overdue ? 'text-red-600 font-medium' : 'text-gray-900' }}">
                                        {{ $cloth->formatted_receive_date ?? 'N/A' }}
                                        @if($cloth->is_overdue)
    ...
    @endif
                    ...
                    <!-- Main Form -->
                    <form method="POST" action="{{ route('customers.store') }}" class="space-y-6" id="tailoring-form">
            @csrf
                        ...
                        <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-3 border border-blue-200">
                            ...
                            <div class="relative">
                                <input type="number"
                                       ...
                                       value="{{ isset($selectedData) && $selectedData->count() > 0 ? $selectedData->first()->cloth_rate : '' }}"
                                       required>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center justify-between gap-4 flex-1 ml-4">
                                ...
                                <div class="flex space-x-2">
                                    <button type="button" class="px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 text-black font-semibold rounded-lg hover:from-amber-600 hover:to-orange-600 transform hover:scale-105 transition-all duration-200 shadow-md text-xs">
                                    ...
                                    <button type="submit" class="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-lg hover:from-indigo-700 hover:to-purple-700 transform hover:scale-105 transition-all duration-200 shadow-md text-xs">
                                        <svg class="w-3 h-3 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ isset($selectedData) && $selectedData->count() > 0 ? 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z' : 'M12 6v6m0 0v6m0-6h6m-6 0H6' }}"></path>
                                        </svg>
            ...
            <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border border-white/20">
                ...
                <div class="p-6">
                    ...
                    <div id="measurement-sections">
                        @if(isset($selectedData) && $selectedData->count() > 0)
                                ...
                                <div class="individual-phone-field">
                                    ...
                                    <div class="relative">
                                        <input type="tel"
                                            ...
                                            placeholder="(xxx) xxx-xxxx"
                                            maxlength="15"
                                            value="{{ $cloth->customer->phone->pho_no ?? '' }}"
                                            required>
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502ل4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
                            ...
                            <div>
                                ...
                                <select name="cloth_size[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" required>
                                    ...
                                    <option value="l" {{ ($cloth->size === 'l') ? 'selected' : '' }}>{{ __('Large') }}</option>
                                    <option value="s" {{ ($cloth->size === 's') ? 'selected' : '' }}>{{ __('Small') }}</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="Height[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" placeholder="{{ __('Enter height') }}" value="{{ $cloth->Height ?? '' }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="Shoulder[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" placeholder="{{ __('Enter shoulder measurement') }}" value="{{ $cloth->Shoulder ?? '' }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="Sleeve[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" placeholder="{{ __('Enter sleeve measurement') }}" value="{{ $cloth->Sleeve ?? '' }}" required>
                </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="Collar[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" placeholder="{{ __('Enter collar measurement') }}" value="{{ $cloth->Collar ?? '' }}" required>
                            </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="chati[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" placeholder="{{ __('Enter chati measurement') }}" value="{{ $cloth->chati ?? '' }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="Armpit[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" placeholder="{{ __('Enter armpit measurement') }}" value="{{ $cloth->Armpit ?? '' }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="Skirt[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" placeholder="{{ __('Enter skirt measurement') }}" value="{{ $cloth->Skirt ?? '' }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="Trouser[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" placeholder="{{ __('Enter trouser measurement') }}" value="{{ $cloth->Trousers ?? '' }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="pacha[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" placeholder="{{ __('Enter leg opening measurement') }}" value="{{ $cloth->Pacha ?? '' }}" required>
                </div>
            </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="flex space-x-1">
                                    <input type="text" name="Kaff[]" list="kaff-options" class="flex-1 px-1 py-2 border border-gray-300 rounded focus:ring-1 focus:ring-emerald-400 focus:border-emerald-400 text-xs bg-white hover:bg-gray-50" placeholder="{{ __('Select kaff type') }}" value="{{ $cloth->Kaff ?? '' }}" required>
                                    <input type="text" name="size_kaf[]" class="w-10 px-1 py-3 border border-gray-300 rounded focus:ring-1 focus:ring-emerald-400 focus:border-emerald-400 text-xs bg-white hover:bg-gray-50" placeholder="{{ __('Size') }}" value="{{ $cloth->size_kaf ?? '' }}">
                                </div>
                            ...
                            <div>
                                ...
                                <div class="flex space-x-2">
                                    <input type="text" name="sleeve_type[]" list="sleeve-options" class="flex-1 px-1 py-2 border border-gray-300 rounded focus:ring-1 focus:ring-emerald-400 focus:border-emerald-400 text-xs bg-white hover:bg-gray-50" placeholder="{{ __('Select sleeve type') }}" value="{{ $cloth->sleeve_type ?? '' }}" required>
                                    <input type="text" name="size_sleve[]" class="w-10 px-1 py-2 border border-gray-300 rounded focus:ring-1 focus:ring-emerald-400 focus:border-emerald-400 text-xs bg-white hover:bg-gray-50" placeholder="{{ __('Size') }}" value="{{ $cloth->size_sleve ?? '' }}">
                                </div>
                            ...
                            <div>
                                ...
                                <select name="Kalar[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" required>
                                    <option value="" {{ empty($cloth->Kalar) ? 'selected' : '' }} disabled>{{ __('Select') }}</option>
                                    <option value="هندی" {{ ($cloth->Kalar ?? '') === 'هندی' ? 'selected' : '' }}>هندی</option>
                                    <option value="قاسمی" {{ ($cloth->Kalar ?? '') === 'قاسمی' ? 'selected' : '' }}>قاسمی</option>
                                    <option value="سینه پور" {{ ($cloth->Kalar ?? '') === 'سینه پور' ? 'selected' : '' }}>سینه پور</option>
                                    <option value="کالر" {{ ($cloth->Kalar ?? '') === 'کالر' ? 'selected' : '' }}>کالر</option>
                                    <option value="پوره بین" {{ ($cloth->Kalar ?? '') === 'پوره بین' ? 'selected' : '' }}>پوره بین</option>
                                    <option value="نیمه بین" {{ ($cloth->Kalar ?? '') === 'نیمه بین' ? 'selected' : '' }}>نیمه بین</option>
                                    <option value="عریی" {{ ($cloth->Kalar ?? '') === 'عریی' ? 'selected' : '' }}>عریی</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="Shalwar[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" required>
                                    <option value="" {{ empty($cloth->Shalwar) ? 'selected' : '' }} disabled>{{ __('Select') }}</option>
                                    <option value="ساده" {{ ($cloth->Shalwar ?? '') === 'ساده' ? 'selected' : '' }}>ساده</option>
                                    <option value="نیمه گیبی" {{ ($cloth->Shalwar ?? '') === 'نیمه گیبی' ? 'selected' : '' }}>نیمه گیبی</option>
                                    <option value="پول گیبی" {{ ($cloth->Shalwar ?? '') === 'پول گیبی' ? 'selected' : '' }}>پول گیبی</option>
                                    <option value="سه درزه" {{ ($cloth->Shalwar ?? '') === 'سه درزه' ? 'selected' : '' }}>سه درزه</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="Daman[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50" required>
                                    <option value="" {{ empty($cloth->Daman) ? 'selected' : '' }} disabled>{{ __('Select') }}</option>
                                    <option value="گول" {{ ($cloth->Daman ?? '') === 'گول' ? 'selected' : '' }}>گول</option>
                                    <option value="چهار کنج" {{ ($cloth->Daman ?? '') === 'چهار کنج' ? 'selected' : '' }}>چهار کنج</option>
                                    <option value="دوه اینچ قات" {{ ($cloth->Daman ?? '') === 'دوه اینچ قات' ? 'selected' : '' }}>دوه اینچ قات</option>
                                    <option value="دری اینچ قات" {{ ($cloth->Daman ?? '') === 'دری اینچ قات' ? 'selected' : '' }}>دری اینچ قات</option>
                                </select>
                            </div>
                    ...
                    <div>
                                ...
                                <input type="text" id="jeb-input" name="Jeb[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white hover:bg-gray-50 mb-2" placeholder="{{ __('Selected jeb types') }}" value="{{ $cloth->Jeb ?? '' }}">
                            ...
                            <div class="bg-blue-100 backdrop-blur-md rounded-xl p-4 shadow-inner border border-gray-200">
                                ...
                                <div class="">
                                    ...
                                    <input type="date" name="Order_date[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white/80 hover:bg-gray-50" value="{{ $cloth->O_date ?? '' }}" required>
                    </div>
                    ...
                    <div>
                                    ...
                                    <input type="date" name="Receive_date[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white/80 hover:bg-gray-50" value="{{ $cloth->R_date ?? '' }}" required>
                                </div>
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 mt-4">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <input list="codeList" id="codeInput" name="cust_name[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white/80 hover:bg-gray-50" placeholder="{{ __('code ...') }}" autocomplete="off">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <input type="number" name="Rate[]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 text-sm bg-white/80 hover:bg-gray-50" placeholder="{{ __('rate') }}" value="{{ $cloth->cloth_rate ?? '' }}">
                                    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Total, Paid, Remain calculation is now handled above
