...
@section('title', 'Vest Assignments Management')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div class="mb-4 md:mb-0">
                <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ __('Vest Assignments Management') }}</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8 animate-fade-in">
        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-2xl shadow-lg p-6 text-white card-hover">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-blue-100 text-sm font-medium">{{ __('Total Vest Assignments') }}</p>
                    <p class="text-3xl font-bold">{{ ($pendingCustomers->count() ?? 0) + ($completeCustomers->count() ?? 0) }}</p>
                </div>
        ...
        <div class="bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl shadow-lg p-6 text-white card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">{{ $completeCustomers->count() ?? 0 }}</p>
                </div>
                <div class="bg-white/20 rounded-xl p-3">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
        ...
        <div class="bg-gradient-to-r from-yellow-500 to-orange-600 rounded-2xl shadow-lg p-6 text-white card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">{{ $pendingCustomers->count() ?? 0 }}</p>
                </div>
                <div class="bg-white/20 rounded-xl p-3">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-r from-purple-500 to-pink-600 rounded-2xl shadow-lg p-6 text-white card-hover">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 text-sm font-medium">{{ __('Total Value') }}</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:scale-105">
            <div class="bg-gradient-to-r from-yellow-500 to-orange-600 p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-2xl font-bold mb-2">{{ __('Pending Vest Assignments') }}</h3>
                        <p class="text-yellow-100">{{ __('Manage vest work that needs to be assigned') }}</p>
                    </div>
            ...
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-sm text-gray-500">{{ __('pending vest assignments') }}</span>
                </div>
                <a href="{{ route('vest-assignments.pending') }}" 
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:scale-105">
            <div class="bg-gradient-to-r from-green-500 to-emerald-600 p-6 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-2xl font-bold mb-2">{{ __('Complete Vest Assignments') }}</h3>
                        <p class="text-green-100">{{ __('View completed vest work assignments') }}</p>
                    </div>
                                            <div class="bg-white/20 rounded-xl p-4">
                            <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-sm text-gray-500">{{ __('completed vest assignments') }}</span>
                    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-900">{{ __('Recent Vest Assignments Overview') }}</h2>
            <p class="text-gray-600 mt-1">{{ __('Quick view of recent vest assignments') }}</p>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    <div class="h-10 w-10 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center justify-center text-white font-semibold text-sm">
                                        #{{ $cust->cus_id }}
                                    </div>
                                </div>
                                <div class="ml-4">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if($cust->vestMeasurements->count() > 0)
                                @php
                                    $latestAssignment = $cust->vestMeasurements->flatMap->clothAssignments->sortByDesc('assigned_at')->first();
                                @endphp
                                {{ $latestAssignment ? $latestAssignment->assigned_at->diffForHumans() : 'Recently added' }}
                            @else
                                {{ __('Recently added') }}
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('vest-assignments.pending') }}" class="text-blue-600 hover:text-blue-900">
                                {{ __('View Details') }}
                            </a>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-gray-500">
                            {{ __('No pending vest assignments found') }}
                        </td>
                   </tr>
                    @endforelse
                </tbody>
