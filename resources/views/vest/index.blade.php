...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-orange-50 py-8 px-4">
        ...
        <div class="bg-white rounded-3xl shadow-2xl p-6 mb-8 border border-white/20 backdrop-blur-sm">
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-600 text-lg">{{ __('Manage all vest tailoring orders') }}</p>
                </div>
            ...
            <div class="bg-green-50 border-l-4 border-green-500 p-4 rounded-lg shadow-md mb-6">
                <div class="flex items-center justify-between">
                    ...
                    @if(session('invoice_id'))
                        <a href="{{ route('vests.print', session('invoice_id')) }}" target="_blank" class="px-4 py-2 bg-gradient-to-r from-orange-600 to-red-600 text-white font-semibold rounded-lg hover:from-orange-700 hover:to-red-700 transform hover:scale-105 transition-all duration-200 shadow-lg flex items-center gap-2">
        ...
        <div class="bg-gradient-to-r from-teal-500 to-cyan-600 rounded-2xl shadow-lg p-6 text-white card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">{{ $vestMeasurements->total() }}</p>
                </div>
        ...
        <div class="bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl shadow-lg p-6 text-white card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">{{ $vestMeasurements->where('Status', 'complete')->count() }}</p>
                </div>
                <div class="bg-white/20 rounded-xl p-3">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-r from-yellow-500 to-orange-600 rounded-2xl shadow-lg p-6 text-white card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">{{ $vestMeasurements->where('Status', 'pending')->count() }}</p>
                </div>
                <div class="bg-white/20 rounded-xl p-3">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-2xl shadow-lg p-6 text-white card-hover">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">{{ $vestMeasurements->whereNotNull('clothAssignments')->count() }}</p>
                </div>
                <div class="bg-white/20 rounded-xl p-3">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 table-hover">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($vestMeasurements as $measurement)
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('vests.show', $measurement) }}" 
                                ...
                                <a href="{{ route('vests.edit', $measurement) }}" 
                                ...
                                <!-- Print Button -->
                                <a href="{{ route('vests.print-size', $measurement->V_M_ID) }}" target="_blank"
                                ...
                                <form method="POST" action="{{ route('vests.destroy', $measurement) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('No vest measurements found') }}</h3>
                                <p class="text-gray-500 mb-4">{{ __('Get started by adding your first vest measurement.') }}</p>
                                <a href="{{ route('vests.create') }}" 
        ...
        @if($vestMeasurements->hasPages())
        <div class="px-6 py-4 border-t border-gray-200">
            {{ $vestMeasurements->links() }}
        </div>
        @endif
    </div>
</div>
