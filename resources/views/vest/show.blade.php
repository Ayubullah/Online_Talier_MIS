    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <a href="{{ route('vests.print-size', $vest->V_M_ID) }}" target="_blank"
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-indigo-500 to-blue-600 px-6 py-8 text-center">
                    <div class="w-24 h-24 mx-auto mb-4 rounded-full bg-white/20 flex items-center justify-center text-white text-2xl font-bold">
                        {{ strtoupper(substr($vest->customer->cus_name, 0, 2)) }}
                    </div>
                    <h2 class="text-2xl font-bold text-white mb-2">{{ $vest->customer->cus_name }}</h2>
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                            ...
                            <span class="text-sm font-semibold text-gray-900">#{{ $vest->V_M_ID }}</span>
                        </div>

                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                            <span class="text-sm font-medium text-gray-500">{{ __('Phone Number') }}</span>
                            @if($vest->customer->phone)
                                <span class="text-sm font-semibold text-gray-900">{{ $vest->customer->phone->pho_no }}</span>
                            @else
                        ...
                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                            ...
                            <span class="text-sm font-semibold text-gray-900">{{ $vest->size_display }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between py-3 border-b border-gray-100">
                            ...
                            <span class="text-lg font-bold text-blue-600">${{ number_format($vest->vest_rate, 2) }}</span>
                        </div>

                        <div class="flex items-center justify-between py-3">
                            <span class="text-sm font-medium text-gray-500">{{ __('Status') }}</span>
                            <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full
                                {{ $vest->Status === 'complete' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ ucfirst($vest->Status) }}
                            </span>
                        </div>
                    </div>
        ...
        <div class="lg:col-span-3">
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-blue-600">${{ number_format($vest->vest_rate, 2) }}</p>
                        </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold {{ $vest->Status === 'complete' ? 'text-green-600' : 'text-yellow-600' }}">
                                {{ ucfirst($vest->Status) }}
                            </p>
                        </div>
                        <div class="{{ $vest->Status === 'complete' ? 'bg-green-100' : 'bg-yellow-100' }} rounded-xl p-3">
                            <svg class="w-6 h-6 {{ $vest->Status === 'complete' ? 'text-green-600' : 'text-yellow-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                @if($vest->Status === 'complete')
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        ...
                        @if($vest->customer && $vest->customer->cus_id)
                        <a href="{{ route('customers.show', $vest->customer->cus_id) }}"
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900">{{ $vest->customer->cus_name }}</p>
                        </div>
                        <div>
                            <h4 class="text-sm font-medium text-gray-500 mb-2">{{ __('Phone Number') }}</h4>
                            @if($vest->customer->phone)
                                <p class="text-lg font-semibold text-gray-900">{{ $vest->customer->phone->pho_no }}</p>
                            @else
                        ...
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-900">#{{ $vest->customer->cus_id }}</p>
                        </div>
                        <div>
                            <h4 class="text-sm font-medium text-gray-500 mb-2">{{ __('Invoice ID') }}</h4>
                            @if($vest->customer->invoice)
                                <p class="text-lg font-semibold text-gray-900">#{{ $vest->customer->invoice->inc_id }}</p>
                            @else
                    ...
                    <div class="mt-6 p-4 bg-blue-50 rounded-lg">
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <span class="font-semibold">${{ number_format($vest->customer->invoice->total_amt ?? 0, 2) }}</span>
                            </div>
                            <div>
                                <span class="text-blue-600">{{ __('Status') }}:</span>
                                <span class="font-semibold">{{ ucfirst($vest->customer->invoice->status ?? 'unknown') }}</span>
                            </div>
                        </div>
                    </div>
                    @endif
            ...
            @if($vest->clothAssignments->count() > 0)
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        @foreach($vest->clothAssignments as $assignment)
            ...
            <div class="bg-white rounded-2xl shadow-xl p-6">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('cloth-measurements.create', ['customer' => $vest->customer->cus_id]) }}"
                    ...
                    <a href="{{ route('customer-payments.create', ['customer_id' => $vest->customer->cus_id]) }}"
